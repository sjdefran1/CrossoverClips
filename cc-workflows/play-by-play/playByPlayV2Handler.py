...
def getPlayByPlayV2Json(game_id: str):
    try:
        url = f"https://stats.nba.com/stats/playbyplayv2?EndPeriod=9&GameID={game_id}&StartPeriod=1"
...
def get_all_stats_V2(gameID, m, d, y):

    response = getPlayByPlayV2Json(game_id=gameID)
    # print(response)
    start = perf_counter()
    plays_dic = {"FGM": 0, "AST": 0, "BLK": 0, "DUNK": 0, "STL": 0}
    players_dic = {"FGM": 0, "AST": 0, "BLK": 0, "DUNK": 0, "STL": 0}

    fgm = playByPlayV2Urls(
        response=response, game_id=gameID, m=m, d=d, y=y, stat_type="FGM"
    )
    ...
    sleep(1)
    dunk = playByPlayV2Urls(
        response=response, game_id=gameID, m=m, d=d, y=y, stat_type="DUNK"
    )
    players_dic["DUNK"] = dunk["players"]
    plays_dic["DUNK"] = dunk["plays"]

    sleep(1)
    ast = playByPlayV2Urls(
        response=response, game_id=gameID, m=m, d=d, y=y, stat_type="AST"
    )
    players_dic["AST"] = ast["players"]
    plays_dic["AST"] = ast["plays"]

    sleep(1)
    blk = playByPlayV2Urls(
        response=response, game_id=gameID, m=m, d=d, y=y, stat_type="BLK"
    )
    players_dic["BLK"] = blk["players"]
    plays_dic["BLK"] = blk["plays"]

    sleep(1)
    stl = playByPlayV2Urls(
        response=response, game_id=gameID, m=m, d=d, y=y, stat_type="STL"
    )
...
def handle_FGM_or_DUNK(action: dict, video_url: str, get_dunks=0) -> tuple:
    ...
    player = (action[15], action[14], action[13])
...
def handle_STL(action: dict, video_url: str):
    ...
    player = (action[22], action[21], action[20])
    return (update_val, player)
...
def handle_BLK(action: dict, video_url: str):
    ...
    player = (action[29], action[28], action[27])
    return (update_val, player)
...
if __name__ == "__main__":
    ...
    print(get_all_stats_V2(gameID=game_id, y=y, m=m, d=d))
